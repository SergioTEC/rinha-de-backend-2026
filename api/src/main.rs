...
fn main() {
    ...
    for w in 0..n {
        // Strip ".sock" suffix, append "-w{N}.sock" so we get /tmp/sock/api1-w0.sock
        // instead of /tmp/sock/api1.sock-w0.
        let sock = if n == 1 {
            sock_path.clone()
        } else {
            let trimmed = sock_path.strip_suffix(".sock").unwrap_or(&sock_path);
            format!("{}-w{}.sock", trimmed, w)
        };
