...
pub struct Dataset {
    ...
    /// Number of IVF cells
    pub num_cells: usize,
    /// Labels: true = fraud, false = legit
    pub labels: Vec<bool>,
    /// IVF centroids: [cell_count][DIMS]
    ...
    /// Dimensions as Structure-of-Arrays: dims[dim_idx][vector_idx]
    pub dims: Vec<Vec<i16>>,
}
...
impl Dataset {
    ...
    /// Uses mmap to avoid double-buffering.
    pub fn load_from_bin(path: &str) -> Self {
        ...
        // Memory map the file
        let mmap = unsafe { memmap2::MmapOptions::new().map(&file).expect("Failed to mmap index") };
        let data = &mmap[..];
        ...
        // Read labels
        let labels: Vec<bool> = (0..count).map(|i| data[offset + i] != 0).collect();
        offset += count;
        ...
        // Read vectors in SoA format
        let mut dims: Vec<Vec<i16>> = Vec::with_capacity(DIMS);
        for d in 0..DIMS {
            let mut dim_vec = Vec::with_capacity(count);
            for i in 0..count {
    ...
    pub fn distance(
        ...
        distance_impl(query, idx, &self.dims)
    }
    ...
    pub fn knn_brute_force(
        ...
        let fraud_count = best.iter().filter(|(_, idx)| self.labels[*idx]).count();
...
#[inline(always)]
fn distance_impl(query: &[i16; DIMS], idx: usize, dims: &[Vec<i16>]) -> i32 {
    #[cfg(has_c_avx2)]
    {
        let mut qa = [0i16; 16];
        let mut ra = [0i16; 16];
    ...
    #[cfg(not(has_c_avx2))]
    {
        let mut sum: i32 = 0;
        for d in 0..DIMS {
            let diff = query[d] as i32 - dims[d][idx] as i32;
            sum += diff * diff;
        }
        sum
    }
...
mod tests {
    ...
    fn test_load_and_search() {
        ...
        ds.count = 3;
        ds.num_cells = 2;
        ds.labels = vec![false, true, false];
        ds.centroids = vec![[0i16; DIMS], [0i16; DIMS]];
        ds.cell_meta = vec![(0, 2), (2, 1)];
        ds.cell_indices = vec![0u32, 1, 2];
        ds.dims = vec![
            vec![1000i16, 9000i16, 2000i16],
            vec![0i16; 3], vec![0i16; 3], vec![0i16; 3],
            vec![0i16; 3], vec![0i16; 3], vec![0i16; 3],
            vec![0i16; 3], vec![0i16; 3], vec![0i16; 3],
            vec![0i16; 3], vec![0i16; 3], vec![0i16; 3], vec![0i16; 3],
        ];
