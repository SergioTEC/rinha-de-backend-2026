...
pub fn fast_path(v: &[f32; 14]) -> FastResult {
    ...
    if unknown_merchant < 0.5 && amount < 0.15 && km_home < 0.2 && is_online < 0.5 {
        return FastResult::Legit;
    }
    
    // Very low MCC risk, known merchant, reasonable distance
    if mcc_risk < 0.15 && unknown_merchant < 0.5 && amount < 0.25 {
        return FastResult::Legit;
    }
    ...
    if is_online > 0.5 && unknown_merchant > 0.5 && mcc_risk > 0.6 && amount > 0.5 {
        return FastResult::Fraud;
    }
    
    // Card not present, far from home, unknown, high amount
    if card_present < 0.5 && km_home > 0.5 && unknown_merchant > 0.5 && amount > 0.4 {
        return FastResult::Fraud;
    }
